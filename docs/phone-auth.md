...
To use phone auth in an expo app, add the `@react-native-firebase/auth` config plug-in to the `plugins` section of your `app.json`. This is in addition to the `@react-native-firebase/app` plugin.
...
The `@react-native-firebase/auth` config plugin is not required for all auth providers, but it is required to use phone auth. The plugin will set up reCAPTCHA verification for you on iOS.
...
[](#mfa-like-account-creation)MFA-like Account Creation
-------------------------------------------------------
